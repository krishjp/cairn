...
MOCK_HIKES = [
    ...
    {"name": "John Muir Trail", "match_level": 0.85, "notes": "A bit crowded but worth it."},
    {"name": "Upper Yosemite Fall Trail", "match_level": 0.95, "notes": "The view from the top is insane."},
    {"name": "Lower Yosemite Fall Trail", "match_level": 1.0, "notes": "Quick morning stroll to the base."},
    {"name": "Four Mile Trail", "match_level": 0.90, "notes": "Steep but the views at the top are unbeatable."},
    {"name": "Panorama Trail", "match_level": 0.80, "notes": "Long day but worth every step."},
    {"name": "Half Dome Trail", "match_level": 0.40, "notes": "Didn't have permits for the cables today."},
]
...
def attach_mock_data(user_id: uuid.UUID):
    ...
    with Session(engine) as session:
        ...
        for hike in MOCK_HIKES:
            ...
            stmt = select(CanonicalRoute).where(CanonicalRoute.name.ilike(f"%{hike['name']}%"))
            route = session.exec(stmt).first()
            
            if not route:
                print(f"Warning: Canonical route '{hike['name']}' not found in DB. Skipping.")
                continue
            ...
            activity = Activity(
                ...
                sport_type="Hike",
                distance=activity_shape.length * 111320, # Rough conversion to meters for mock
...
def detach_mock_data(user_id: uuid.UUID):
    with Session(engine) as session:
        ...
        stmt = delete(Activity).where(Activity.user_id == user_id).where(Activity.name.like("[MOCK]%"))
        result = session.exec(stmt)
        session.commit()
        
        print(f"Removed mock activities.")
